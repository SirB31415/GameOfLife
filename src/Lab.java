    ...
    //remember that you need to watch for the edges of the array
	  ...
	  for(int i = 0; i < grid.length; i++)
	  {
		  for(int j = 0; j < grid[i].length; j++)
		  {
			  ...
			  if(i+1 < grid.length && grid[i+1][j]==METAL)
			  {
				  numberOfNeighbors++;
			  }
			  if(i > 0 && grid[i-1][j]==METAL)
			  {
				  numberOfNeighbors++;
			  }
			  if(j+1 < grid[i].length && grid[i][j+1]==METAL)
			  {
				  numberOfNeighbors++;
			  }
			  if(j > 0 && grid[i][j-1]==METAL)
			  {
				  numberOfNeighbors++;
			  }
			  if(i+1 < grid.length && j+1 < grid[i].length && grid[i+1][j+1]==METAL)
			  {
				  numberOfNeighbors++;
			  }
			  if(j > 0 && i+1 < grid.length && grid[i+1][j-1]==METAL)
			  {
				  numberOfNeighbors++;
			  }
			  if(i > 0 && j+1 < grid[i-1].length && grid[i-1][j+1]==METAL)
			  {
				  numberOfNeighbors++;
			  }
			  if(i > 0 && j > 0 && grid[i-1][j-1]==METAL)
			  {
					  numberOfNeighbors++;
			  }
